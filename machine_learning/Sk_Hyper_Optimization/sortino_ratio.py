...
class Backtester:
    ...
    @staticmethod
    def calculate_sortino_ratio(buy_signals: list, sell_signals: list, risk_free_rate=0.0) -> float:
        returns = [(sell - buy) / buy * 100 for buy, sell in zip(buy_signals, sell_signals)]
        risk_free_returns = [risk_free_rate] * len(returns)
        downside_returns = [min(0, x - y) for x, y in zip(returns, risk_free_returns)]
    ...
    def evaluate_strategy(self, dataframe: pd.DataFrame) -> tuple:
        ...
        sortino_ratio = self.calculate_sortino_ratio(self.buy_signals, self.sell_signals)
        profit = self.calculate_profit(self.buy_signals, self.sell_signals)
        return -sortino_ratio, sortino_ratio, profit
    ...
    def find_optimal_parameters(self, params: list) -> float:
        ...
        neg_sortino, sortino_ratio, profit = self.evaluate_strategy(filtered_dataframe)
        rounded_sortino = round(sortino_ratio, 2)
        rounded_profit = round(profit, 2)

        # Check if the current Sortino ratio is greater than the highest_sortino
        if sortino_ratio > self.highest_profit:
            self.highest_profit = sortino_ratio
            self.best_profit = profit  # Add this line
            print("Attempt with rsi_buy_threshold={}, rsi_sell_threshold={}, sortino_ratio={}, profit={}%".format(
                buy_rsi, sell_rsi, rounded_sortino, rounded_profit))
        return neg_sortino


    def optimize_trading_strategy(self, find_optimal_parameters, search_space):  # Change method name
        result = gp_minimize(
        ...
        if not best_dataframe.empty:
            _, best_sortino, best_profit = self.evaluate_strategy(best_dataframe)
            message = "Best parameters: rsi_buy_threshold={}, rsi_sell_threshold={}, sortino_ratio={}, profit={}%".format(
                best_params[0], best_params[1], round(best_sortino, 2), round(best_profit, 2))
        else:
            best_sortino = self.highest_profit
            best_profit = self.best_profit
            message = "Best parameters: rsi_buy_threshold={}, rsi_sell_threshold={}, sortino_ratio={}, profit={}%".format(
                best_params[0], best_params[1], round(best_sortino, 2), round(best_profit, 2))

        return best_params, best_sortino, message
