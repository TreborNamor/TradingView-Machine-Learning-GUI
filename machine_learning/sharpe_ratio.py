...
class Backtester:
    ...
    def __init__(self, dataframe):
        """
            ...
            Attributes:
                ...
                best_profit (float): The profit corresponding to the highest Sharpe ratio obtained during optimization.
        """
    ...
    @staticmethod
    def calculate_sharpe_ratio(buy_signals: list, sell_signals: list, risk_free_rate=0.0) -> float:
        """
        ...
        sharpe_ratio = expected_excess_return / excess_return_std
        return sharpe_ratio
    ...
    def evaluate_strategy(self, dataframe: pd.DataFrame) -> tuple:
        ...
        sharpe_ratio = self.calculate_sharpe_ratio(self.buy_signals, self.sell_signals)
        profit = self.calculate_profit(self.buy_signals, self.sell_signals)
        return -sharpe_ratio, sharpe_ratio, profit
    ...
    def find_optimal_parameters(self, params: list) -> float:
        ...
        neg_sharpe, sharpe_ratio, profit = self.evaluate_strategy(filtered_dataframe)
        rounded_sharpe = round(sharpe_ratio, 4)
        rounded_profit = round(profit, 2)

        # Check if the current Sharpe ratio is greater than the highest_profit
        if sharpe_ratio > self.highest_profit:
            self.highest_profit = sharpe_ratio
            self.best_profit = profit
            print("Attempt with rsi_buy_threshold={}, rsi_sell_threshold={}, sharpe_ratio={}, profit={}%".format(
                buy_rsi, sell_rsi, rounded_sharpe, rounded_profit))
        return neg_sharpe
    ...
    def optimize_trading_strategy(self, find_optimal_parameters, search_space):
        ...
        if not best_dataframe.empty:
            _, best_sharpe, best_profit = self.evaluate_strategy(best_dataframe)
            message = "Best parameters: rsi_buy_threshold={}, rsi_sell_threshold={}, sharpe_ratio={}, profit={}%".format(
                best_params[0], best_params[1], round(best_sharpe, 4), round(best_profit, 2))
        else:
            best_sharpe = self.highest_profit
            best_profit = self.best_profit
            message = "Best parameters: rsi_buy_threshold={}, rsi_sell_threshold={}, sharpe_ratio={}, profit={}%".format(
                best_params[0], best_params[1], round(best_sharpe, 4), round(best_profit, 2))

        return best_params, best_sharpe, message
