...
class Backtester:
    ...
    def __init__(self, dataframe):
        """
            ...
            Attributes:
                ...
                highest_profit (float): The highest Information ratio obtained during optimization.
                best_profit (float): The profit corresponding to the highest Information ratio obtained during optimization.
        """
    ...
    @staticmethod
    def calculate_information_ratio(buy_signals: list, sell_signals: list, benchmark_returns: list) -> float:
        """
            Calculate the Information ratio for a trading strategy.
        ...
        excess_returns = [x - y for x, y in zip(returns, benchmark_returns)]
        ...
        information_ratio = expected_excess_return / tracking_error
    ...
    def calculate_profit(self, buy_signals: List[float], sell_signals: List[float]) -> float:
        """
            Calculates the Information ratio for the given buy and sell signals and market returns.
            ...
            Returns:
                Tuple[float, float, float]: A tuple containing the negative Information ratio, Information ratio, and profit.
        """
        profits = [sell_price / buy_price - 1 for buy_price, sell_price in zip(buy_signals, sell_signals)]
    ...
    def evaluate_strategy(self, dataframe: pd.DataFrame) -> tuple:
        """
            Evaluate the trading strategy by calculating the Information ratio and profit based on the buy and sell signals and market returns.

            Args:
                dataframe (pd.DataFrame): The data on which the trading signals will be evaluated.
            Returns:
                A tuple of three values: the negative Information ratio, the positive Information ratio, and the profit percentage.
        """
        ...
        # Calculate simple returns for the benchmark ('SPY' index)
        benchmark_returns = self.dataframe['close'].pct_change().dropna().tolist()

        # Match the length of the benchmark_returns with the buy and sell signals
        benchmark_returns = benchmark_returns[-len(self.buy_signals):]

        information_ratio = self.calculate_information_ratio(self.buy_signals, self.sell_signals, benchmark_returns)
        profit = self.calculate_profit(self.buy_signals, self.sell_signals)
        return -information_ratio, information_ratio, profit
    ...
    def find_optimal_parameters(self, params: list) -> float:
        """
            ...
            Returns:
                The negative Information ratio of the trading strategy.
        """
        ...
        neg_information_ratio, information_ratio, profit = self.evaluate_strategy(filtered_dataframe)
        rounded_information_ratio = round(information_ratio, 4)
        rounded_profit = round(profit, 2)

        # Check if the current Information ratio is greater than the highest_profit
        if information_ratio > self.highest_profit:
            self.highest_profit = information_ratio
            self.best_profit = profit
            print("Attempt with rsi_buy_threshold={}, rsi_sell_threshold={}, information_ratio={}, profit={}%".format(
                buy_rsi, sell_rsi, rounded_information_ratio, rounded_profit))
        return neg_information_ratio
    ...
    def optimize_trading_strategy(self):
        """
            ...
            Returns:
                A tuple of three values: the best parameters for the trading strategy, the best Information ratio, and a message displaying the best parameters and evaluation metrics.
        """
        result = gp_minimize(
        ...
        if not best_dataframe.empty:
            _, best_information_ratio, best_profit = self.evaluate_strategy(best_dataframe)
            message = "Best parameters: rsi_buy_threshold={}, rsi_sell_threshold={}, information_ratio={}, profit={}%".format(
                best_params[0], best_params[1], round(best_information_ratio, 4), round(best_profit, 2))
        else:
            best_information_ratio = self.highest_profit
            best_profit = self.best_profit
            message = "Best parameters: rsi_buy_threshold={}, rsi_sell_threshold={}, information_ratio={}, profit={}%".format(
                best_params[0], best_params[1], round(best_information_ratio, 4), round(best_profit, 2))

        return best_params, best_information_ratio, message
